...
// Importamos los tipos EXACTOS de tu archivo central
import { User, DietType, WorkoutFocus } from '../types';

// Definimos los detalles de la dieta usando el Enum corregido
  ...
  if (!user || !user.nutrition) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f8fafc]">
        <p className="text-2xl font-black italic animate-pulse text-slate-300 uppercase">Cargando Datos IA...</p>
      </div>
    );
  }

  // Buscamos la dieta del usuario o usamos OMNIVORE por defecto si hay error
  const diet = DIET_DETAILS[user.diet_type] || DIET_DETAILS[DietType.OMNIVORE];

  const macros = [
    { label: 'PROTEÍNA', val: user.nutrition.protein, max: 250, color: 'bg-emerald-400' },
    { label: 'CARBOS', val: user.nutrition.carbs, max: 400, color: 'bg-blue-400' },
    { label: 'GRASA', val: user.nutrition.fat, max: 120, color: 'bg-orange-400' },
  ];
            ...
            <div className="space-y-4">
              {['STRENGTH', 'HYPERTROPHY', 'METABOLIC'].map((f) => (
                ...
                >
                  ...
                  <span className="text-[10px] font-black opacity-40 uppercase">INICIAR →</span>
                </button>
              ))}
            </div>
          ...
          <div className="col-span-12 lg:col-span-3">
            <div className="bg-white rounded-[55px] p-10 shadow-xl border border-slate-50 flex flex-col items-center">
              ...
              <div className="bg-[#10b981] w-full p-8 rounded-[38px] text-white shadow-lg text-center">
                <p className="text-[9px] font-black opacity-70 mb-1 uppercase tracking-widest">LOGROS</p>
          ...
          <div className="col-span-12 lg:col-span-9 space-y-10">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-10 rounded-[45px] border-2 border-orange-500 shadow-sm text-center">
                <p className="text-[9px] font-black text-slate-300 mb-1 uppercase tracking-widest">ESTADO</p>
            ...
            <div className="grid grid-cols-12 gap-8">
              ...
              <div className="col-span-12 md:col-span-4 bg-white rounded-[50px] p-10 shadow-xl border border-slate-100 flex flex-col">
                <p className="text-[9px] font-black text-slate-400 mb-6 uppercase tracking-[0.2em]">MACRONUTRIENTES</p>
                  ...
                  {macros.map(m => (
                    <div key={m.label} className="space-y-2">
                      ...
                      <div className="h-2.5 bg-slate-50 rounded-full overflow-hidden border border-slate-100">
                        <div 
                          className={`h-full ${m.color} transition-all duration-1000 shadow-inner`} 
                ...
                <div className="mt-auto p-6 bg-slate-50 rounded-[35px] border border-dashed border-slate-200 text-center">
                   <p className="text-[9px] font-black text-slate-400 uppercase mb-2">MENU RECOMENDADO</p>
